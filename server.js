...
// PROCESS EDIT PAGE
app.post("/edit/:animalID", function(request, response) {
  console.log("updated animal is", request.body);
  console.log("animal id is", request.params);
...
app.delete("/rmv/:animalID", function(request, response) {
  console.log("animal deleted"); // ...delete 1 record by a certain key/value.
  Animal.remove({ _id: request.params.animalID }, function(err, animalID) {
    response.redirect("/"); // This code will run when the DB has attempted to remove one matching record to {_id: 'insert record unique id here'}
  });
});
